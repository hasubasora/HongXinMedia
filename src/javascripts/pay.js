...
const outMoney = {
    init() {
        ...
        const outMoneyNum = document.querySelector('.outMoneyNum')
        const rep = /\d+\.\d+/ig;
        ...
        //支付宝的确定按钮
        $('.GoPay').on('click', () => {
            let releCq = filter.unBlank($('.releCq').val());
            let zfbPay = filter.unBlank($('.zfbPay').val());
            if (releCq && zfbPay) {
                //检测都存在，就发送ajax
                layer.open({
